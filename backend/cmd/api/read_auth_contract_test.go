...
// readSources は main.go と routes.go の内容を結合して返す。
// ルート定義が routes.go に移動したため、禁止パターンの検査対象はどちらのファイルにも適用する。
func readSources(t *testing.T) string {
	...
	if err != nil {
		t.Fatalf("routes.go を読み込めません: %v", err)
	}
	return string(main) + string(routes)
}

func TestAPIRoutesDoNotAllowAnonymousReadAuth(t *testing.T) {
	source := readSources(t)
	...
	if strings.Contains(source, "OptionalAuth") || strings.Contains(string(middlewareSource), "OptionalAuth") {
...
func TestFreeAPIKeyRouteIsDisabled(t *testing.T) {
	...
	if strings.Contains(source, `POST("/free-apikeys"`) {
...
func TestIdolBulkCreateRouteIsDisabled(t *testing.T) {
	...
	if strings.Contains(source, `POST("/bulk"`) {
...
func TestIdolUpdateUsesSinglePatchRoute(t *testing.T) {
	...
	if !strings.Contains(source, `PATCH("/:id"`) {
...
func TestIdolDuplicateCandidateRouteIsDisabled(t *testing.T) {
	...
	if strings.Contains(source, "duplicate-candidates") {
